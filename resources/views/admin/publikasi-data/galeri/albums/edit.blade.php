  ...
  <div class="py-8" data-theme="light">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="mb-4 flex items-center justify-between">
        ...
        <span class="text-xs text-zinc-500">Form edit album galeri</span>
      </div>
      ...
      <div class="bg-white shadow-sm sm:rounded-lg p-6">
        ...
        <form id="form-galeri-album"
              action="{{ route('admin.publikasi-data.galeri.albums.update', $album) }}"
          ...
          <div class="grid md:grid-cols-3 gap-6">
            <div class="md:col-span-2">
              ...
              <input id="judul" name="judul" type="text"
                     ...
                     value="{{ old('judul', $album->judul) }}" placeholder="Contoh: Dokumentasi Quran Terjemahan Bahasa Gorontalo">
            ...
            <div class="md:col-span-1">
              ...
              <input id="slug" name="slug" type="text"
                     class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                     value="{{ old('slug', $album->slug) }}" placeholder="otomatis dari judul, bisa diubah">
              @error('slug') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
              <p class="mt-1 text-[11px] text-zinc-500">
                Digunakan di URL, misal: <span class="font-mono">/galeri/album/<span id="slugPreview">{{ old('slug', $album->slug) ?: 'nama-album' }}</span></span>
              </p>
            </div>
          </div>
          ...
          <div class="grid md:grid-cols-4 gap-6">
            <div class="md:col-span-2">
              ...
              @php 
                $kategoriOld = old('kategori', $album->kategori);
                $isManualKategori = !in_array($album->kategori, ['Dokumentasi Bahasa','Quran Terjemahan BG','Dikili & Sastra','Kegiatan Pusat Studi','Pelatihan & Workshop','Kunjungan & Kolaborasi']);
              @endphp
              <select id="kategori" name="kategori"
                ...
                @foreach (['Dokumentasi Bahasa','Quran Terjemahan BG','Dikili & Sastra','Kegiatan Pusat Studi','Pelatihan & Workshop','Kunjungan & Kolaborasi'] as $opt)
                  <option value="{{ $opt }}" @selected($kategoriOld === $opt && !$isManualKategori)>{{ $opt }}</option>
                @endforeach
              </select>
              <p class="mt-1 text-[11px] text-zinc-500">
              ...
              <input id="kategori_manual" name="kategori_manual" type="text"
                     ...
                     value="{{ $isManualKategori ? old('kategori_manual', $album->kategori) : old('kategori_manual') }}" 
                     placeholder="Atau ketik kategori manual di sini">
            ...
            <div>
              ...
              <input id="tahun" name="tahun" type="number" min="1900" max="2100"
                     class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                     value="{{ old('tahun', $album->tahun) }}" placeholder="{{ now()->year }}">
            ...
            <div>
              ...
              <input id="urutan" name="urutan" type="number" min="0"
                     class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                     value="{{ old('urutan', $album->urutan) }}" placeholder="0">
          ...
          <div class="grid md:grid-cols-4 gap-6">
            <div>
              ...
              <input id="tanggal_mulai" name="tanggal_mulai" type="date"
                     class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                     value="{{ old('tanggal_mulai', $album->tanggal_mulai ? $album->tanggal_mulai->format('Y-m-d') : '') }}">
            ...
            <div>
              ...
              <input id="tanggal_selesai" name="tanggal_selesai" type="date"
                     class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                     value="{{ old('tanggal_selesai', $album->tanggal_selesai ? $album->tanggal_selesai->format('Y-m-d') : '') }}">
            ...
            <div class="md:col-span-2">
              ...
              <input id="lokasi" name="lokasi" type="text"
                     class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                     value="{{ old('lokasi', $album->lokasi) }}" placeholder="Contoh: Kampus UNG, Gorontalo / Masjid Agung Baiturrahim">
          ...
          <div>
            ...
            <textarea id="deskripsi_singkat" name="deskripsi_singkat" rows="2"
                      class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      placeholder="Ringkasan album dalam 1–2 kalimat untuk ditampilkan di kartu galeri.">{{ old('deskripsi_singkat', $album->deskripsi_singkat) }}</textarea>
          ...
          <div>
            ...
            <textarea id="deskripsi" name="deskripsi" rows="5"
                      class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      placeholder="Boleh diisi narasi lebih panjang tentang konteks album, kegiatan, atau koleksi.">{{ old('deskripsi', $album->deskripsi) }}</textarea>
            @error('deskripsi') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
          </div>

          {{-- Cover album --}}
          <div x-data="{ preview: '{{ $album->cover_path ? Storage::url($album->cover_path) : '' }}' }">
            ...
            @if($album->cover_path)
              <div class="mb-3">
                <p class="text-xs text-zinc-500 mb-1">Cover saat ini:</p>
                <img src="{{ Storage::url($album->cover_path) }}" alt="Cover saat ini" class="max-h-56 rounded border border-zinc-200 bg-white p-1">
                <p class="text-xs text-zinc-500 mt-1">{{ basename($album->cover_path) }}</p>
              </div>
            @endif
            ...
            <input id="cover" name="cover" type="file" accept="image/*"
                   ...
                   @change="
                     ...
                     if (!f) { preview = '{{ $album->cover_path ? Storage::url($album->cover_path) : '' }}'; return; }
            ...
            <template x-if="preview">
              <div class="mt-3">
                <p class="text-xs text-zinc-500 mb-1">Preview cover baru:</p>
            ...
            @if($album->cover_path)
              <div class="mt-2 flex items-center gap-2">
                <input id="hapus_cover" name="hapus_cover" type="checkbox" value="1"
                       class="h-4 w-4 rounded border-zinc-300 text-red-600 focus:ring-red-500">
                <label for="hapus_cover" class="text-sm text-red-700">
          ...
          <div class="grid md:grid-cols-2 gap-6">
            <div>
              <label for="is_published" class="block text-sm font-medium text-zinc-700">Status Publikasi</label>
              <select id="is_published" name="is_published"
                      class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                <option value="1" @selected(old('is_published', $album->is_published) == 1)>Publik</option>
                <option value="0" @selected(old('is_published', $album->is_published) == 0)>Draft</option>
              </select>
              @error('is_published') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
              <label for="published_at" class="block text-sm font-medium text-zinc-700">Tanggal Publikasi</label>
              <input id="published_at" name="published_at" type="datetime-local"
                     class="mt-1 block w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                     value="{{ old('published_at', $album->published_at ? $album->published_at->format('Y-m-d\TH:i') : '') }}">
          ...
          <div class="flex items-center gap-2">
            <input id="tampil_beranda" name="tampil_beranda" type="checkbox" value="1"
                   ...
                   @checked(old('tampil_beranda', $album->tampil_beranda))>
          ...
          <div class="flex items-center justify-between pt-6 border-t border-zinc-200">
            <div class="text-sm text-zinc-500">
              Dibuat: {{ $album->created_at->translatedFormat('d F Y H:i') }}
              @if($album->updated_at->ne($album->created_at))
                <br>Diupdate: {{ $album->updated_at->translatedFormat('d F Y H:i') }}
              @endif
            </div>
        ...
        <div class="mt-8 pt-6 border-t border-red-200">
          <div class="bg-red-50 p-4 rounded-md">
            <h3 class="text-sm font-medium text-red-800">Area Berbahaya</h3>
            <p class="text-sm text-red-600 mt-1">
              Hapus permanen album ini beserta semua media di dalamnya. Tindakan ini tidak dapat dibatalkan.
            </p>
            <form action="{{ route('admin.publikasi-data.galeri.albums.destroy', $album) }}" method="POST" class="mt-3"
                  onsubmit="return confirm('Apakah Anda yakin ingin menghapus album ini? Semua media di dalamnya juga akan dihapus.')">
              @csrf
              @method('DELETE')
              <button type="submit"
  ...
  @push('scripts')
    <script>
      ...
      (function(){
        ...
        // Handle kategori manual logic
        const $kategori = document.getElementById('kategori');
        const $kategoriManual = document.getElementById('kategori_manual');
        
        if ($kategori && $kategoriManual) {
          $kategori.addEventListener('change', function() {
            if (this.value) {
              $kategoriManual.value = '';
            }
          });
          
          $kategoriManual.addEventListener('input', function() {
            if (this.value) {
              $kategori.value = '';
            }
          });
