<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
      {{ __('Menu Fasilitas - Edit Fasilitas') }}
    </h2>
  </x-slot>
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white shadow-sm sm:rounded-lg p-6">
        ...
        <div class="mb-6">
          ...
          <p class="text-gray-600 mt-2">Update informasi fasilitas</p>
        </div>

        <form action="{{ route('admin.fasilitas.update', $fasilitas->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
          @csrf
          @method('PUT')
            ...
            <div class="flex items-center justify-center w-full">
              <label for="gambar" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 @error('gambar') border-red-500 @enderror">
                <div class="flex flex-col items-center justify-center pt-5 pb-6" id="upload-placeholder">
                  ...
                  <p class="text-xs text-blue-500 mt-2">Kosongkan jika tidak ingin mengganti gambar</p>
                </div>
                <input id="gambar" name="gambar" type="file" class="hidden" accept="image/*" />
                <div id="image-preview" class="hidden w-full h-full p-4">
                  <img id="preview-image" class="w-full h-full object-contain rounded-lg" src="" alt="Preview gambar baru">
          ...
          <div>
            ...
            <input type="text" 
                   name="alt_text" 
                   id="alt_text"
                   value="{{ old('alt_text', $fasilitas->alt_text) }}"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="flex items-center space-x-3">
                <div class="flex items-center">
                  <input type="checkbox" 
                         ...
                         id="tampil_beranda"
                         value="1"
                         {{ old('tampil_beranda', $fasilitas->tampil_beranda) ? 'checked' : '' }}
            ...
            <div>
              ...
              <input type="number" 
                     name="urutan_tampil" 
                     id="urutan_tampil"
                     value="{{ old('urutan_tampil', $fasilitas->urutan_tampil) }}"
                     min="0"
          ...
          <div class="bg-gray-50 rounded-lg p-4 mt-6">
            <h3 class="text-sm font-medium text-gray-700 mb-2">Informasi Sistem</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
              <div>
                <span class="font-medium">Dibuat:</span> 
                {{ $fasilitas->created_at->translatedFormat('d F Y H:i') }}
              </div>
              <div>
                <span class="font-medium">Diupdate:</span> 
                {{ $fasilitas->updated_at->translatedFormat('d F Y H:i') }}
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between pt-6 border-t border-gray-200">
            <div>
              <button type="button" 
                      onclick="if(confirm('Hapus fasilitas ini?')) { document.getElementById('delete-form').submit(); }"
                      class="px-4 py-2 text-sm font-medium text-red-700 bg-red-50 border border-red-200 rounded-md shadow-sm hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                Hapus Fasilitas
              </button>
            </div>
        ...
        {{-- Form Delete Hidden --}}
        <form id="delete-form" action="{{ route('admin.fasilitas.destroy', $fasilitas->id) }}" method="POST" class="hidden">
          @csrf
          @method('DELETE')
        </form>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      fileInput.addEventListener('change', function(e) {
        ...
        if (file) {
          ...
          // Jika file dihapus, tampilkan placeholder lagi
      ...
      deleteButton.addEventListener('click', function() {
        if (confirm('Apakah Anda yakin ingin menghapus fasilitas "{{ $fasilitas->nama_fasilitas }}"? Tindakan ini tidak dapat dibatalkan.')) {
